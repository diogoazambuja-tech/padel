...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      ...
      if(pl&&pl.length>0){
        ...
        const {data:inserted}=await supabase.from('players').insert(DEMO).select();
        setPlayers(inserted||DEMO);
      }
      if(gm)setGames(gm);
      setLoading(false);
    })();
  ...
  const delPlayer=async id=>{
    if(games.some(g=>[...g.team1,...g.team2].includes(id))){alert("Jogador tem jogos registados.");return;}
  ...
  return(
    <div className="app">
      ...
      <main className="main">
        ...
        {tab==="team"&&<TeamTab players={players} onAdd={addPlayer} onSave={savePlayer} onDel={delPlayer} games={games}/>}
...
function TeamTab({players,onAdd,onSave,onDel,games}){
